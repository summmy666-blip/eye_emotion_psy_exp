...
class EmotionAnalyzer:
    ...
    def find_image_pairs(self, experience_folder, neutral_folder):
        """查找两个文件夹中的匹配图片对 - 从Gemma代码中移植的文件名检索方式"""
        image_pairs = {}
        ...
        try:
            ...
            for filename in experience_files:
                ...
                # 提取ID和情绪
                # 新的文件名格式: cropped_12_Caucasian_female_disgusted.jpg
                parts = filename.split('_')
                if len(parts) < 3:  # 确保有足够的部分
                    continue

                # ID在第二个位置 (cropped_12_...)
                subject_id = parts[1]

                # 情绪在文件名的最后部分，去掉扩展名
                last_part = parts[-1].split('.')[0].lower()
                if last_part in self.emotions and last_part != "neutral":
                    # 查找对应的中性图片 (cropped_12_Caucasian_female_neutral.jpg)
                    neutral_filename = None
                    for nf in neutral_files:
                        if nf.startswith(f"cropped_{subject_id}_") and "neutral" in nf.lower():
                            neutral_filename = nf
                            break
...
def main():
    # 使用指定的文件路径
    experience_folder = "/home/majc_docker/sun/eyebrow/"
    neutral_folder = "/home/majc_docker/sun/eyebrow_neutral/"
    ...
    # 生成结果文件
    for file_num in range(1, 37):  # 可以调整范围
        # 记录开始时间
