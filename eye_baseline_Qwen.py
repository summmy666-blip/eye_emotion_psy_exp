    ...
    def find_image_pairs(self, experience_folder, neutral_folder):
        ...
        try:
            ...
            for filename in experience_files:
                ...
                # 提取ID和情绪
                # 新的文件名格式: cropped_151_159_Rafd090_12_Caucasian_female_angry_frontal.jpg
                parts = filename.split('_')
                if len(parts) < 8:  # 确保有足够的部分
                    continue

                # ID在第5个位置 (cropped_151_159_Rafd090_12_...)
                subject_id = parts[4]

                # 情绪在倒数第二个部分
                emotion_type = parts[-2].lower()
                if emotion_type in self.emotions and emotion_type != "neutral":
                    # 查找对应的中性图片，格式替换情绪词为neutral
                    neutral_filename = None
                    for nf in neutral_files:
                        if (nf.startswith(f"cropped_") and f"_{subject_id}_" in nf and 
                            "neutral" in nf.lower() and nf.split('_')[-1] == filename.split('_')[-1]):
                            neutral_filename = nf
                            break
...
def main():
    # 使用指定的文件路径
    experience_folder = "/home/majc_docker/sun/eyes/"
    neutral_folder = "/home/majc_docker/sun/eyes_neutral/"
