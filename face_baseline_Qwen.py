...
class EmotionAnalyzer:
    def __init__(self, num_workers=1):
        ...
        self.prompts = {
            "description": """
            这组图片包含两张面孔/两双眼睛。第一张展示的是中性情绪，第二张展示的是同一个人表达的六个基本情绪之一。
    ...
    def api_request(self, prompt: str, neutral_path: str, emotion_path: str, prompt_type: str, subject_id: str) -> Optional[str]:
        ...
        messages.append({
            "role": "system",
            "content": [{"type": "text", "text": "你是一个专业的情绪分析助手，根据提供的中性图片和情绪图片进行分析。请保持简洁明了的回答。"}]
        })
        ...
        if subject_id not in self.chat_history:
            ...
            neutral_message = {
                ...
                "content": [
                    ...
                    {"type": "text", "text": "这是一个人的中性表情照片，请记住这个人的中性表情特征，后续我们将分析这个人的其他情绪表情。"}
                ]
            }
            neutral_response = {
                "role": "assistant",
                "content": [{"type": "text", "text": "好的，我已记住这个人的中性表情特征，准备分析后续的情绪表情。"}]
            }
            ...
            for filename in experience_files:
                ...
                if last_part in self.emotions and last_part != "neutral":
                    ...
                    for nf in neutral_files:
                        if nf.startswith(subject_id + "_") and "neutral" in nf.lower():
                            neutral_filename = nf
                            break
                    ...
                    if neutral_filename:
                        ...
                        # 添加情绪图片路径
                        image_pairs[subject_id][last_part] = os.path.join(experience_folder, filename)
...
def main():
    # 使用原来的文件路径
    experience_folder = "/home/majc_docker/sun/face/"
    neutral_folder = "/home/majc_docker/sun/face_neutral/"
    ...
    # 生成结果文件
    for file_num in range(19, 20):  # 可以调整范围
        # 记录开始时间
