...
class EmotionAnalyzer:
    ...
    def process_single_image(self, image_path: str, emotion_label: str) -> Dict:
        ...
        # 获取中文形容词描述
        adj_prompt = "这张图片包含两对眼睛。上方眼睛展示的是基线表情（中性表情），下方眼睛展示的是情绪表情。请参考上方的基线表情，分析下方眼睛表达了什么神情？明确用两个字的形容词表述，不要有多余的话，另外不要回答无法判断，实在判断不了你就随便说"

        # 获取情绪判断
        emotion_prompt = """
        你是一名情绪识别专家。你将看到一张包含两对眼睛的图像：
        上方眼睛展示的是基线表情（中性表情），下方眼睛展示的是情绪表情。
        你的任务是参考上方的基线表情，识别下方眼睛表达的情绪。
    ...
    def find_image_pairs(self, experience_folder: str) -> List[Dict]:
        image_pairs = []
        experience_pattern = re.compile(r"cropped_\d+_\w+_\w+_(angry|disgusted|fearful|happy|sad|surprised)\.jpg$")
...
def main():
    ...
    # 保存结果
    excel_output_path = "combined_emotion_results27.xlsx" # 修改输出文件名
